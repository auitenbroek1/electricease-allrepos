...
var updatestatus = 1;
var partsList;
var DistributorAssembliesDataTable;

$(document).ready(function () {
    $("#Loading").show();
    Cookies.set('DAssemdtpageno', "");
    Cookies.set('DAssemdtpagelen', "");
    Cookies.set('DAssemsearch', "");
    $('li').removeClass('active');
    $("#distributorassembly").addClass('active');
    $(".content-title").text("Distributor Assemblies Master");
    ...
    getassemblyInfo('');
    //GetJsonDistributorAssemblyMasterList();
    loadGrid();
    ...
    $('#ClientPartsTable').on('click', 'tbody input[type="checkbox"]', function (e) {
...
function OpenPartsPopup() {
    ...
    if (validateBeforePopup()) {
        ...
        $('#ClientPartsTable').DataTable().columns().search('').draw();
...
function loadGrid() {
    $("#Loading").hide();
    DistributorAssembliesDataTable = $('#DistributorAssemblyMasterDataTable').DataTable({
        ...
        "ajax": {
            url: "../DistributorAssembly/GetDistributorAssembliesGrid",
            type: 'GET',
            datatype: "json"
        },
        "filterDropDown": {
        ...
        "columns": [
            ...
            },
            {
                ...
                mRender: function (data, type, row) {
                    var actionControls = '<a href="javascript:void(0);" onclick="editassembly(\'' + row.Assemblies_Name + '\',\'' + row.Distributor_Id + '\')">'
                        + '<span class="glyphicon glyphicon-pencil"></span></a>';

                    actionControls = actionControls.concat('<a href="javascript:void(0);" onclick="duplicateAssembly(\'' + row.Assemblies_Name + '\',\'' + row.Distributor_Id + '\')">'
                        + '<span class="glyphicon glyphicon-file"></span></a>');

                    if (row.IsActive === true) {
                        actionControls = actionControls.concat('<a href="javascript:void(0);" onclick="deleteAssembly(\'' + row.Assemblies_Name + '\',\'' + row.Distributor_Id + '\')">'
...
function loadPartsPopupGrid() {
    $('#ClientPartsTable').DataTable().destroy();
    ClientPartsTable = $('#ClientPartsTable').DataTable({
        ...
        "orderCellsTop": true,
        "ajax": {
            url: "../DistributorAssembly/GetPartsListByDistributorID",
...
function validateBeforePopup() {
    if ($("#Assemblies_Name").val().trim() === "" || $("#DistributorID").val() === "" || $("#Assemblies_Category").val() === "" || $("#Assemblies_Description").val().trim() === "") {
...
function getassemblyInfo(val, val1) {
    $.ajax({
        url: '../DistributorAssembly/DistributorAssemblyInfo?Name=' + encodeURIComponent(val) + '&Did=' + val1,
        type: 'GET',
        success: function (data) {
            ...
            if ($("#DistributorID").val() != "") {
                LoadDistributorAssembliesCategorydropdown($("#hidAssembliescategory").val());
                loadPartsPopupGrid();
            }
...
function getassemblyList(val, showBot) {
    if (val == null || typeof val === 'undefined')
        val = "";
    $("#DAssembliesTbl").DataTable().ajax.reload(null, false);
    if (showBot == 1) {
        $("#divDisAssBot").focus();
    }
    //$.ajax({
    //    url: '../DistributorAssembly/getallAssembliesList?assembName=' + val,
    //    type: 'GET',
    //    success: function (data) {
...
function modalAssignToRM() {
    ...
    var D_id = $("#DistributorID").val();
    if (D_id != "") {
        var lstpartnum = "";
        var tablRows = $('#dpartstbody tr');
        i = 0;

        if (tablRows.length > 0) {
            $('#dpartstbody tr ').each(function () {
                var partNumber = $(this).find("#partnumber").html();
                lstpartnum += partNumber.trim() + ",";
                selectedpart.push(partNumber.trim());
            });
        }
        if ($('#dpartstbody tr').length > 0) {
            updatestatus = 1;
        }
        $.ajax({
            url: '../DistributorAssembly/GetallAssemblyParts',
            data: { name: assemblyname, lstparts: lstpartnum, Did: D_id },
            type: 'POST',
            success: function (data) {
                ...
                //$.each(data, function () {
                //    PartsPopup = PartsPopup + ('<tr><td>' + this.isChekced + '</td>'
                //        + '<td>' + this.Part_Category + '</td>'
                //        + '<td>' + this.Description + '</td>'
                //        + '<td>' + this.Part_Number + '</td>'
                //        + '<td>' + this.Cost + '</td>'
                //        + '<td>' + this.Resale_Cost + '</td></tr>');
                //});



                //$('#partsbody').html(PartsPopup);
                

                //$('#partstable').DataTable({
                //        columns: [
                //        { data: "data.isChekced" },
                //        { data: "data.Part_Category" },
                //        { data: "data.Description" },
                //        { data: "data.Part_Number" },
                //        { data: "data.Cost" },
                //        { data: "data.Resale_Cost" }
        ...
        $("#Loading").hide();
        DisplayNotification("Please select \"Distributor\" name", "error");
    }
...
function populateDataTable(data) {
    ...
    $.each(data, function () {
        $('#partstable').dataTable().fnAddData([
            this.isChekced,
            this.Part_Category,
            this.Description,
            this.Part_Number,
...
function ShowOtherCategory(val) {
    ...
    var D_id = $("#DistributorID").val();
    $.ajax({
        url: '../DistributorAssembly/GetallAssemblyParts?PartCatgory=' + val + "&lstparts=" + lstpartnum + "&Did=" + D_id,
        type: 'Get',
        success: function (data) {
            $('#divpartsdata').html(data);
            if (val == "") {
...
function saveparts() {
    ...
    var D_id = $("#DistributorID").val();
    var PartstblRows = $('#dpartstbody tr');
    var oldlabour = null;
    ...
    $.ajax({
        url: '../DistributorAssembly/SelectedAssemblyPartsLaborInfo',
        data: { lstparts: lstpart, Assemblies_Name: AsName, Did: D_id },
        type: 'POST',
        success: function (data) {

            //to enable and disable the distributor dropdown
            if (data.partslist.length > 0) {
            ...
            for (var i = 0; i < data.partslist.length; i++) {
                ...
                for (var j = 0; j < oldPartsData.length; j++) {
                    ...
                    if (data.partslist[i].Part_Number == oldPartsData[j].Part_Number) {
                        ...
                        EstQty = Number(EstQty).toFixed(0);
                        trow += '<input type="text" name="txtEstimatedQty" id="' + EstQtyid + '" class="form-control" value="' + EstQty + '" onkeyup="checkqtynum(this);calculatecost(this);" style="text-align:right" />';
                        trow += '</td><td>';

                        //Labor Unit
                        trow += '<input type="text" name="txtLaborUnit" value="' + oldPartsData[j].LaborUnit + '" class="form-control" onkeyup="checknum(this);calculatecost(this);" style="text-align:right" />';
                        trow += '</td><td>';
                ...
                if (isavailable == 0) {
                    ...
                    //Resale Cost
                    var Rcostid = "P_" + i + "_1";
                    trow += '<input name="txtResaleCost" type="text"  ' + disabled + ' id="' + Rcostid + '" class="form-control" value="' + data.partslist[i].Resale_Cost + '" onkeyup="checknum(this);calculatecost(this);" style="text-align:right" />';
                    trow += '</td><td>';
                    ...
                    EstQty = Number(EstQty).toFixed(0);
                    trow += '<input type="text" name="txtEstimatedQty" ' + disabled + ' id="' + EstQtyid + '" class="form-control" value="' + EstQty + '"onkeyup="checkqtynum(this);calculatecost(this);" style="text-align:right" />';
                    trow += '</td><td>';
...
function calculatecost(val) {
    ...
    if (partsrow.length > 0) {
        for (var i = 0; i < partsrow.length; i++) {
            ...
            //if (qty == 0)
            //    qty = "";
            partsrow[i].cells[6].children[0].value = qty;
    ...
    $('#assmeblymasterinfo_Estimated_Qty_Total').val(Number(EstQty_Total));
...
function saveassembly() {
    ...
    if (validateAssemblyInformation()) {
        ...
        //if (($("#Assemblies_Name").val() != "" && $("#DistributorID").val()!="" && $("#Assemblies_Description").val() != "") && $('#PartsLabortblInfo >tbody >tr').length > 0 && nameValidate($("#Assemblies_Name").val()) == true) {
        var model = new Array();
        ...
        if (tablRows.length > 0) {
            ...
            for (i = 0; i < tablRows.length; i++) {
                ...
                //    isvalid = false;
                //    errormsg = "Please check the added Parts Cost,Resale Cost and Qty. All fields are required, 0 is not allowed";
                //}
                //else {
                //    if (Number($(tablRows).find('#' + "P_" + i + "_0").val()) > Number($(tablRows).find('#' + "P_" + i + "_1").val())) {
                //        isvalid = false;
                //        errormsg = "Please check the added Parts, Cost should not be greater than Resale Cost. (Check Part Number " + $(tablRows[i]).find('#partnumber').text().trim() + " )";
                //    }
                //}
        ...
        if (isvalid == true) {
            ...
            if (labortablRows.length > 0) {
                $(labortablRows).each(function (index, row) {
                    ...
                    //    isvalid = false;
                    //    errormsg = "Please check the Labor values, all fields are required and 0 is not allowed";
                    //}
                    //else {
                    ...
                    //        isvalid = false;
                    //        errormsg = "Please check the Labor, Cost should not be greater than  or Equal to Resale Cost";
                    //    }
                    //}
            ...
            if (isvalid == true) {
                ...
                if (confirm(msg)) {
                    ...
                    $.ajax({
                        url: '../DistributorAssembly/AddAssembliesDetails',
                        data: formData,
                        type: 'POST',
                        processData: false,
                        contentType: false,
                        success: function (data) {
                            if (data == "“Assemblies details” are added successfully.") {
                                ...
                                getassemblyInfo('', '');
                                DistributorAssembliesDataTable.ajax.reload(null, false);
...
function validateAssemblyInformation() {
    ...
    if ($("#Assemblies_Category").val() == "0" || $("#Assemblies_Category").val() == "other") {
...
function UpdateAssembly() {
    ...
    var formData = new FormData();
    if (($("#Assemblies_Name").val() != "" && $("#DistributorID").val() != "" && $("#Assemblies_Description").val() != "") && ($("#Assemblies_Category").val() != 0 || ($("#Assemblies_Category").val() == 1 && $("#OtherAssemblies_Category").val() != "")) && $('#PartsLabortblInfo >tbody >tr').length > 0) {
        ...
        if (tablRows.length > 0) {
            ...
            for (i = 0; i < tablRows.length; i++) {
                ...
                //    if (Number($(tablRows).find('#' + "P_" + i + "_0").val()) >= Number($(tablRows).find('#' + "P_" + i + "_1").val())) {
                //        isvalid = false;
                //        errormsg = "Please check the added Parts, Cost should not be greater than or equal to Resale Cost. (Check Part Number " + $(tablRows[i]).find('#partnumber').text().trim() + " )";
                //    }
                //}
        ...
        if (isvalid == true) {
            ...
            if (labortablRows.length > 0) {
                ...
                //    if (Number($(labortablRows).find("#assmeblymasterinfo_labor_cost").val()) >= Number($(labortablRows).find("#assmeblymasterinfo_Lobor_Resale").val())) {
                //        isvalid = false;
                //        errormsg = "Please check the Labor, Cost should not be greater than equal to Resale Cost";
                //    }
                //}
                $(labortablRows).each(function (index, row) {
            ...
            if (isvalid == true) {
                ...
                if (confirm(msg)) {
                    formData.append('model', JSON.stringify(model));
                    $.ajax({
                        url: '../DistributorAssembly/UpdateAssembliesDetail',
                        data: formData,
                        type: 'POST',
                        processData: false,
                        contentType: false,
...
function deleteAssembly(val, val1) {
    ...
    if (confirm(msg)) {
        $.ajax({
            url: '../DistributorAssembly/DeleteAssembliesDetails?AssembliesName=' + encodeURIComponent(val) + '&Did=' + val1,
            type: 'Get',
            success: function (data) {
...
//    if (document.getElementById('DistributorID').selectedIndex == 0) {
...
function PreASDescriptionInPopUp() {
    $.ajax({
        url: '../DistributorAssembly/GetPreviousDescriptionList',
        type: "Get",
        success: function (data) {
...
function Cancel() {
    ...
    if (confirm(msg)) {
        clear();
        //window.location.href = '../DistributorAssembly/Index'
        return true;
    }
    else {
        return false;
...
function setRegexValidations() {
    ...
    $("[name = 'txtEstimatedQty']").limitkeypress({ rexp: /^(\d{1,4})?$/ });
