...
const Proposal = (props: any) => {
  ...
  const [to, setTo] = useState<any>(data?.email)
  const [cc, setCC] = useState<any>(``)
  const [subject, setSubject] = useState<any>(``)

  //

  const [attachments, setAttachments] = useState<any>([`proposal`])

  const attachmentOptions = [
    { id: `proposal`, name: `Proposal`, required: true },
  ...
  const handleSend = async () => {
    const endpoint = `/api/jobs/${job.id}/${data.id}/proposal`
  ...
  useEffect(() => {
    if (!job.id) return
    setSubject(`Proposal for ${data.name} - ${job.name}`)
  }, [job?.id, data?.name])
            ...
            <div className={`overflow-hidden`}>
              <div className={`-m-8 h-[600px]`}>
                <iframe
                  ...
                  src={`/emails/jobs/${job.id}/${data.id}/proposal/attachments/proposal`}
                />
              </div>
            </div>
          </Card.Group>
