...
export const Locations = (props: any) => {
  ...
  useEffect(() => {
    console.log(`debug.locations`, `close`)
    setShowNew(false)
  }, [data?.length])
  ...
  const handleDelete = async (id: number) => {
    console.log(`debug.locations`, `delete`, id)
    await axios.delete(`/api/jobs/locations/${id}`)
    reloadJob()
  }
    ...
    >
      <div className="border-b pb-4">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Capture information about the location for the job site
        </p>
      </div>
      <div className="space-y-4">
        {data.length === 0 && <LocationForm />}
        {data.length > 0 && (
          <>
            <div>
