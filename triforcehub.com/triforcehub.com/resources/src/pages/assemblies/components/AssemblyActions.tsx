...
export const AssemblyActions = () => {
  ...
  const handleEdit = async () => {
    console.log(`edit`)
    navigate(`/app/assemblies/all/${item.id}/edit`)
  }

  // Assembly Duplicate
  const handleDuplicate = async () => {
    try {
      const response = await axios.post(`/api/assemblies/${item.id}/duplicate`)
      const data = response.data.data
      navigate(`/app/assemblies/all/${data.id}/edit`)
    } catch (error) {
      console.log(error)
    }
  }

  const { destroy } = useController({ endpoint: `/api/assemblies` })
      ...
      >
        <Modal.Content>
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            <div className="text-lg font-medium leading-6">Delete material</div>
            <div className="mt-2">
