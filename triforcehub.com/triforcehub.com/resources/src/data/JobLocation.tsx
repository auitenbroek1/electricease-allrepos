...
export const useJobLocation = (data: any) => {
  ...
  const onDeleteClick = async () => {
    const response = await axios.delete(`/api/jobs/locations/${id}`)
  ...
  const save = async () => {
    ...
    if (id) {
      ...
      try {
        const response = await axios.patch(`/api/jobs/locations/${id}`, payload)
      ...
      try {
        const response = await axios.post(`/api/jobs/locations`, payload)
        console.log(`debug.save`, response.data.data)
        await reloadJob()
        toast.success(`Saved!`)
      } catch (exception: any) {
