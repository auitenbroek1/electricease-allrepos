  ...
  <body class="text-sm font-sans p-8 text-gray-700">
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="col-span-2">
        <div class="space-y-4">
          <div>
            <span class="text-black font-bold">Terms and Conditions:</span>
          </div>
          <div>
            @foreach ($job['blocks'] as $block)
              @if ($block['job_section_id'] == 3)
                <p class="whitespace-pre-line">{{$block['content']}}</p>
              @endif
            @endforeach
          </div>
      ...
      <div>
        @php
            $total = $job['summary']['adjustments']['summary']['cost_total'];
            $tax = $job['summary']['adjustments']['summary']['tax_total'];
            $subtotal = $total - $tax;
        @endphp
        @if (!$job['settings']['hide_tax_in_proposal'])
          <div class="grid grid-cols-2 gap-4">
            <div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <span class="text-black font-bold">Tax:</span>
            </div>
            <div class="text-right">
              ${{number_format($tax, 2)}}
            </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <span class="text-black font-bold">Grand Total:</span>
          </div>
          <div class="text-right">
