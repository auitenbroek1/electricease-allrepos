...
class MigrateTrinityPowerJobs extends Command
{
    // job_locations name cannot be empty
    // job_labors Column 'hours', 'burden', 'fringe' cannot be null

    protected $signature = 'triforce:migrate-trinity-power-jobs';

    protected $description = 'Migrate Trinity Power Jobs';

    private $member = 'Trinity Power and Controls, LLC';
    ...
    public function handle(): void
    {
        ...
        $source_jobs = $this->db->table('Job_Master')->select([
            ...
            'Directions_To', 'Doing_What',
        ])
            ->whereIn('Client_ID', [$member['source_member_id']])->get();
        ...
        foreach ($source_jobs as $source_job) {
            ...
            $job = Job::factory()->create([
                ...
                'name' => $source_job->Job_Description,
            ...
            foreach ($source_parts as $source_part) {
                $part = Part::where([
                    'name' => $source_part->Description != null ? $source_part->Description : $source_part->Part_Number,
                    'member_id' => $member['member_id'],
                ])->first();
            ...
            foreach ($source_assembly_parts as $source_assembly_part) {
                ...
                if (! $part) {
                    $part = Part::factory()->create([
                        ...
                        'labor' => $source_assembly_part->LaborUnit ?: 1,
                    ]);
                }
