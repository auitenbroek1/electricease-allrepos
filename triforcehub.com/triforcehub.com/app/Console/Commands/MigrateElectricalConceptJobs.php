...
class MigrateElectricalConceptJobs extends Command
{
    ...
    protected $signature = 'triforce:migrate-electrical-concepts-jobs';

    protected $description = 'Migrate Electrical Concepts Jobs';

    private $member = 'Electrical Concepts';
    ...
    // private $job_list = [4571];

    private $job_list = [4528, 4536, 4544, 4542, 4548, 4551, 4566, 4578, 4573, 4569, 4567, 4571, 4512, 4509, 4506, 4452];
    ...
    public function handle(): void
    {
        ...
        foreach ($source_jobs as $source_job) {
            ...
            $job = Job::factory()->create([
                ...
                'number' => $source_job->Job_Number ?: $source_job->Job_ID,
        ...
        echo "Migrating Predefined Assemblies & Its Materials \n";

        $source_selected_assemblies = file_get_contents(database_path('Electrical_Concepts_Assemblies.json'));

        $source_selected_assemblies = json_decode($source_selected_assemblies, true);

        foreach ($source_selected_assemblies as $source_selected_assembly) {
            echo 'Migrating Assemly '.$source_selected_assembly['name'].' - '.$source_selected_assembly['category']."\n";
            ...
            $source_assembly_parts = $this->db->table('Job_Assembly_Parts')
                ...
                ->where('Job_Assembly_Parts.Client_ID', '=', $member['source_member_id'])
                ->where('Job_Assembly_Parts.Assemblies_Name', '=', $source_selected_assembly['name'])
                ->where('Job_Assembly_Parts.Assemblies_Category', '=', $source_selected_assembly['category'])
                ->where('Job_AssembliesDetails.Client_ID', '=', $member['source_member_id'])
                ->where('Parts_Details.Client_ID', '=', $member['source_member_id'])
                ->get();
            ...
            foreach ($source_assembly_parts as $source_assembly_part) {
                ...
                if (! $part_category) {
                    $part_category = PartCategory::factory()->create([
                        ...
                        'is_migrated' => true,
                        'member_id' => $member['member_id'],
                        'name' => $source_assembly_part->Part_Category ?: $source_assembly_part->Distributor_Part_Category,
                    ]);
                }
